...
class ClimateGraphRAG:
    ...
    def __init__(self, graph_json_path: str, original_csv_path: str = None,
        ...
        # --- CHANGE 2: Build the FAISS index at startup ---
    ...
    def _load_graph(self, json_path: str) -> nx.Graph:
        ...
        G = nx.Graph()
        for node in data['nodes']: G.add_node(node['id'], **node)
        for link in data['links']: G.add_edge(link['source'], link['target'], type=link['type'])
    ...
    def _create_embeddings(self) -> Dict[str, np.ndarray]:
        ...
        for node_id, node_data in self.graph.nodes(data=True):
            ...
            if 'name' in node_data: text_parts.append(node_data['name'])
            if 'description' in node_data: text_parts.append(node_data['description'])
            if 'type' in node_data: text_parts.append(f"Type: {node_data['type']}")
    ...
    # --- CHANGE 3: New method to build the FAISS index ---
    def _build_faiss_index(self):
        """Builds a FAISS index from the node embeddings for fast searching."""
        print("🚀 Building FAISS index for fast semantic search...")
        
        self.faiss_id_map = list(self.node_embeddings.keys())
        embedding_matrix = np.array([self.node_embeddings[node_id] for node_id in self.faiss_id_map]).astype('float32')
    ...
    def _create_analytical_cache(self) -> Dict[str, Any]:
        ...
        if self.df is None: return cache
    ...
    # --- CHANGE 4: Rewritten semantic_search to use the FAISS index ---
    def semantic_search(self, query: str, top_k: int = 10) -> List[Tuple[str, float]]:
        """
        Performs a highly efficient semantic search using the pre-built FAISS index.
        """
    ...
    def graph_traversal_search(self, start_nodes: List[str], max_depth: int = 2) -> Dict[str, Any]:
        ...
        for node_id in subgraph_nodes:
            if node_id in self.graph:
                ...
                if node_type not in nodes_by_type: nodes_by_type[node_type] = []
...
Examples:
  ...
  python graphrag_cli.py --interactive --llm openai
    ...
    parser.add_argument('--llm', choices=['openai', 'gemini'], help='LLM provider to use')
    ...
    if args.llm and not args.no_llm:
        ...
        if not llm_provider: print("⚠️  Continuing without LLM...")
...
📚 Available Commands:
  ...
  - 'quit' - Exit the system
                ...
                if query.lower() == 'stats':
                    ...
                    if graph_rag.df is not None: print(f"  - Original cases: {len(graph_rag.df)}")
